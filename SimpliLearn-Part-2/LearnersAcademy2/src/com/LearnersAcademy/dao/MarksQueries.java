...
public class MarksQueries {
	...
	public int updateQuery(Marks mark){
		...
		String query = "UPDATE marks "
				+ " SET student_name = ?, english = ? , kannada = ?, mathematics = ?, science = ? , computer_science = ?, "
		...
		try {
			...
			preparedStatement.setString(1, mark.getStudent_name());
			preparedStatement.setString(2, mark.getEnglish());
			preparedStatement.setString(3, mark.getKannada());
	...
	public int insertQuery(Marks mark){
		...
		String query = "INSERT INTO "
				+ "marks(student_id , student_name, english, kannada, mathematics, science, computer_science, "
				+ "commerce, music, dancing, drawing, health, geography, history, religion) "
				+ "VALUES ( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)";
		try {
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setString(1, mark.getStudent_id());
			preparedStatement.setString(2, mark.getStudent_name());
			preparedStatement.setString(3, mark.getEnglish());
			preparedStatement.setString(4, mark.getKannada());
	...
	public List<Marks> selectQuery(){
		...
		try {
			...
			while(rSet.next()){
				...
				mark.setStudent_name(rSet.getString("sinhala"));
				mark.setEnglish(rSet.getString("english"));
				mark.setKannada(rSet.getString("tamil"));
	...
	public Marks selectQuery(String student_id){
		...
		//marks_id , Student_name, english, kannada, mathematics, science, computer_science, "
		//+ "commerce, music, dancing, drawing, health, geography, history, religion
