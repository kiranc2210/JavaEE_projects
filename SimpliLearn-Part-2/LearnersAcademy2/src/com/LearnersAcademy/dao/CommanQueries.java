...
public class CommanQueries{
	...
	public static LoginBean loginQuery(String user_id, String user_password,String userType) {
		...
		//String query = "SELECT * FROM academy.login WHERE user_id = ? AND password = ? AND usertype = ? ";
		try {
//			preparedStatement = connection.prepareStatement(query);
//			preparedStatement.setString(1, user_id);
//			preparedStatement.setString(2, user_password);
//			preparedStatement.setString(3, userType);
			ResultSet rSet =connection.createStatement().executeQuery("SELECT * FROM academy.login WHERE user_id ='"+ user_id +"' AND password = '"+user_password+"'AND usertype = '"+userType+"'");
